...
export function frequencyTokens(usages: NumericUsage[], prefix: string, minCount = 2): DesignToken[] {
  const grouped = new Map<string, { value: string; px: number | null; count: number; properties: Set<string> }>()
  ...
  const ranked = [...grouped.values()].sort((a, b) => b.count - a.count || (a.px ?? 0) - (b.px ?? 0))
  return ranked
    .map((item, index) => ({
      id: `${prefix}_${index + 1}`,
      name: `${prefix}-${index + 1}`,
