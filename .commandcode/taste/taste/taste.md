...
- Before a Chrome Web Store submission, verify the zipped `dist/` bundle actually contains `background.js` with the real `chrome.scripting` (and any other declared-permission) calls, and zip the _contents_ of `dist/` with `manifest.json` at the zip root (upload ZIP, not `.crx`) — a bundle missing the code behind a declared permission gets auto-rejected for "unused permissions". Confidence: 0.8
- Keep every manifest permission backed by visible usage in the shipped bundle; do not remove a genuinely-needed permission (e.g. `scripting`) just to satisfy a rejection — fix the packaging instead. Confidence: 0.75
- The broad `host_permissions` (`http://*/*`, `https://*/*`) are intentional and required — they back cross-origin asset fetching for the export ZIP (`fallbackContentFetch`) and `declarativeNetRequestWithHostAccess` header-stripping for the responsive preview; keep them and fill in the CWS "Host permission justification" field (emphasizing user-initiated, local-only use) rather than narrowing them. The yellow host-access warning on the dashboard is informational and always appears with `*/*` hosts. Confidence: 0.8
- Writes in brief, informal, non-native English and pastes full source material (e.g. entire rejection emails) when asking for a fix — respond in plain, direct language with a clear root-cause and concrete numbered action steps. Confidence: 0.6
